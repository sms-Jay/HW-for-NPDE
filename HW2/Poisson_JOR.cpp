...
class Poisson{
    ...
    public:
        Poisson(double size):h(size){
            ...
            u.resize(4*N+1);
            for(int i=0;i<=4*N;i++){
                u[i].resize(j_num[i]+1,1.0);
            }
            //记录真实解
            solu.resize(4*N+1);
            for(int i=0;i<=4*N;i++){
                solu[i].resize(j_num[i]+1,1.0);
            }
            for(int i=0;i<=4*N;i++){
        ...
        vector<vector<double>> gs(const vector<vector<double>>& u_old){
            ...
            u_new[0][2*N]=(h*g(2,-2)+A*beta(2,-2)*(2*u_old[0][2*N-1]-u_old[1][2*N-1]))/(A*beta(2,-2)+h*alpha(2,-2));
            //第1至N-1行
            for(int i=1;i<=N-1;i++){
                double y=-2+i*h;
                ...
                for(int j=1;j<j_num[i];j++){
                    double x=j*h;
                    u_new[i][j]=(u_old[i-1][j]+u_old[i][j-1]+u_old[i+1][j]+u_old[i][j+1]+h*h*f(x,y))/4;
                }
                int j=j_num[i];
                double x=j*h;
                u_new[i][j]=(h*g(x,y)+A*beta(x,y)*(u_old[i-1][j]+u_old[i][j-1]))/(2*A*beta(x,y)+h*alpha(x,y));
            }
            //第N至3N-1行
            for(int i=N;i<=3*N-1;i++){
                ...
                u_new[i][j]=(h*g(x,y)+beta(x,y)*B*(2*u_old[i][j-1]+u_old[i+1][j]))/(beta(x,y)*B*3+h*alpha(x,y));
            }
            //第3N行
            u_new[3*N][0]=(h*g(0,1)+beta(0,1)*u_old[3*N][1])/(beta(0,1)+h*alpha(0,1));
            for(int j=1;j<2*N;j++){
                double x=j*h;
                u_new[3*N][j]=(u_old[3*N-1][j]+u_old[3*N][j-1]+u_old[3*N+1][j-1]+u_old[3*N][j+1]+h*h*f(x,1))/4;
            }
            u_new[3*N][2*N]=(h*g(2,1)+A*beta(2,1)*(2*u_old[3*N][2*N-1]-u_old[3*N+1][2*N-2]))/(A*beta(2,1)+h*alpha(2,1));   
            //第3N至4N-1行
            for(int i=3*N+1;i<=4*N-1;i++){
                double y=-2+i*h;
                double x=y-1;
                u_new[i][0]=(h*g(x,y)+A*beta(x,y)*(u_old[i-1][1]+u_old[i][1]))/(2*beta(x,y)*A+h*alpha(x,y));
                for(int j=1;j<j_num[i];j++){
                    double x=j*h+y-1;
                    u_new[i][j]=(u_old[i-1][j+1]+u_old[i][j-1]+u_old[i+1][j-1]+u_old[i][j+1]+h*h*f(x,y))/4;
                }
                int j=j_num[i];
                x=3-y;
                u_new[i][j]=(h*g(x,y)+A*beta(x,y)*(u_old[i-1][j+1]+u_old[i][j-1]))/(2*beta(x,y)*A+h*alpha(x,y));
            }
            //第4N行
            u_new[4*N][0]=(h*g(1,2)+A*beta(1,2)*(2*u_old[4*N-1][1]-u_old[4*N-1][0]))/(A*beta(1,2)+h*alpha(1,2));
            //单次SOR
            for(int i=0;i<=4*N;i++){
                for(int j=0;j<=j_num[i];j++){
        ...
        //2范数误差
        double square_error(const vector<vector<double>>& u_old, const vector<vector<double>>& u_new){
            double error=0.0;
            for(int i=0;i<=4*N;i++){
                for(int j=0;j<=j_num[i];j++){
                    double e=u_new[i][j]-u_old[i][j];
                    error+=e*e;
                }
            }
            error=h*sqrt(error);
        ...
        void SOR(const int max_iter, double tolerance){
            ...
            for(int k=1;k<=max_iter;k++){
                ...
                cout<<error<<endl;
                if(error<tolerance){
                    cout<<"GS converged at "<<k<<endl;
                    error=inf_error(u,solu);
                    cout<<"Error: "<<error<<endl;
                    return;
                }
                u_old=u;
            }
            cout<<"GS not converged"<<endl;
        };
        void print(){
            for(int i=4*N;i>=0;i--){
                for(int j=0;j<=j_num[i];j++){
