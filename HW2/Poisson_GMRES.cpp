...
class Poisson_GMRES{
    ...
    public:
        ...
        double g(double x, double y){
            double ux = cos(PI*x)*cos(2*PI*y)/(5.0*PI);
            double uy = -2.0*sin(PI*x)*sin(2*PI*y)/(5.0*PI);
            double un;
            double tol = 1e-3;
            if((y+2) < tol && x >0 && x < 2) un = -uy;
            else if((y+x)<tol && x>1 && x<2) un = A*(ux+uy);
            else if((y-2*x+3)<tol && x>1 && x<2) un = B*(2*ux-uy);
            else if((y+x-3)<tol && x>1 && x< 2) un = A*(ux+uy);
            else if((y-x-1)<tol && x>0 && x< 1) un = A*(uy-ux);
            else if(x==0 && y>-2 & y<1) un = -ux;
            else cout<<x<<" "<<y<<" Wrong range!"<<endl;
            
            return alpha(x,y)*sin(PI*x)*cos(2*PI*y)/(5.0*PI*PI)+beta(x,y)*un;
        };
                ...
                for(int j = 0;j <= J;j++){
                    if(i == 0){
                        ...
                        if(j == 0){
                            ...
                            if(i != n) v_new[idx] = (h*g(x,y)+A*beta(x,y)*(v_new[idx-1]+v_new[idx-J-2]))/(2*A*beta(x,y)+h*alpha(x,y));
                            else v_new[idx] = real_u(x,y);
                        }
                        else{
                        ...
                        if(j == 0){
                            ...
                            v_new[idx] = (h*g(x,y)+beta(x,y)*B*(2*v_new[idx-1]+v_old[idx+J+1]))/(3*B*beta(x,y)+h*alpha(x,y));
                        }
                        else{
                            if((i-n) % 2 == 0){
                ...
                for(int j = 0;j <= J;j++){
                    if(i == 0){
                        ...
                        if(j == 0){
                            ...
                            if(i != n) z[idx] = alpha(x,y)*v[idx]+beta(x,y)*A*(2*v[idx]-v[idx-1]-v[idx-J-2])/h;
                            else z[idx] = real_u(x,y);
                        }
                        else{
        ...
        vector<double> GMRES(vector<double>& x0, const double epsilon, const double max_iter){
            ...
            for(int k = 1;k <= max_iter;k++){
                ...
                if(residual < epsilon){
                    u = xl;
                    cout<<"When h = "<<h<<" , GMRES converged at "<<k<<" iterations"<<endl;
                    break;
                }
                else{
                    x0 = xl;
                }
                if(k == max_iter) cout<<"GMRES not convergee!"<<endl;
            }
            return u;
        }
        void solve(){
